...
with st.sidebar:
    ...
    n_gpu_layers = st.sidebar.slider('n_gpu_layers', min_value=0.01, max_value=5.0, value=0.1, step=0.01)
    n_batch = st.sidebar.slider('n_batch', min_value=0.01, max_value=5.0, value=0.1, step=0.01)
...
def generate_llm_response(prompt_input):
    ...
    llm = load_model(selected_model, n_gpu_layers, n_batch, temperature_option, max_length_option, top_p_option)
    #Tao chain
